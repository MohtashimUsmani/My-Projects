...
def add_contact():
    ...
    address1 = str(input('Enter contact address:'))
    address_book[name] = {'name': name, 'phone_number': phone_number, 'address': address1}
...
def update_contact():
    ...
    if not address_book:
        ...
        if name in address_book:
            phone = int(input('Enter the updated phone number (Press Enter if you dont want to update number'))
